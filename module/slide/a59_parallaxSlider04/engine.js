bd.slide.A59_parallaxSlider04 = function() {
	this.init.apply(this, arguments);
};

bd.slide.A59_parallaxSlider04.prototype = {
	isReady: false,
	
	init: function() {
		bd.util.addCss(bindobj.moduleroot + '/slide/_common/ParallaxSlider/style.css');
		bd.util.addCss(bindobj.moduleroot + '/slide/a59_parallaxSlider04/style.css');
	...
	render: function( elem, autost, loop ) {
		...
		var slide = jQuery('<div class="pxs_container a59_parallaxSlider04">').css({
			width: w,
			height: h,
		...
		jQuery(window).resize(function(){
			var w = jQuery('.a59_parallaxSlider04').parent().width();
			if (wFlg) w = jQuery(window).width();
			jQuery('.a59_parallaxSlider04').css('width', w + 'px');
		});
	}
};
