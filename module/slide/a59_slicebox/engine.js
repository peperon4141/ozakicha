bd.slide.A59_slicebox = function() {
	this.init.apply(this, arguments);
};

bd.slide.A59_slicebox.prototype = {
	isReady: false,
	...
	render: function( elem, autost, loop ) {
		...
		var sbshadow = jQuery('<div class="sb-shadow2"></div>').appendTo(slidewrap);
		var sbarrows = jQuery('<div class="sb-nav-arrows"></div>').appendTo(slidewrap);
		var sbNext = jQuery('<a href="javascript:void(0);">Next</a>').appendTo(sbarrows);
		var sbPrev = jQuery('<a href="javascript:void(0);">Previous</a>').appendTo(sbarrows);
			...
			if((imgObj != null) && (imgObj.length != 0)){
				...
				if (ttl.length>0) {
					var sbdesc = jQuery('<div class="sb-description"></div>')
						...
						.css('left','5px')
						.css('right','5px')
						.css('padding','10px').appendTo(licntslide);
					jQuery('<h3>'+ttl+'</h3>')
						.css('font-size','12px')
		...
		var Page = (function() {
			var $navArrows = jQuery( '.sb-nav-arrows', slidewrap ).hide(),
				$shadow = jQuery( '.sb-shadow2', slidewrap ).hide(),
